...
def main():
    ...
    train.add_argument('-ld', '--log_dir',
                       type=str, default='',
                       help = 'logfilename = log_dir/log.txt')
    train.add_argument('-md', '--mode', type=str, default='train')
    train.add_argument('-en', '--expnum', type=int, default=0)
    ...
    if len(opt['log_dir']) == 0:
        opt['log_dir'] = 'exp/' + str(opt['expnum'])

    if not os.path.exists(opt['log_dir']):
        os.makedirs(opt['log_dir'])

    logFile = open(opt['log_dir'] + '/log.txt', 'a')
    #opt['logFile'] = logFile  # this type of opt is not serializable
    ...
    while True:
        ...
        if opt['log_every_n_secs'] > 0 and log_time.time() > opt['log_every_n_secs']:
            ...
            if hasattr(train_report, 'get') and train_report.get('total'):
                ...
                if hasattr(agent, 'nll_per_word'):
                    logs.append('nll_per_word = ' + str(agent.nll_per_word))
        ...
        if (opt['validation_every_n_secs'] > 0 and
            ...
            if valid_report['accuracy'] > best_accuracy:
                ...
                impatience += 1
                print('[ did not beat best accuracy: {} impatience: {} ]'.format(round(best_accuracy, 4), impatience))
                print('save agent')
                world.save_agents()
            validate_time.reset()
